# XYZ Bank – Manual Testing Project


## 📌 Project Overview

This project is a Manual Software Testing project for the XYZ Bank web application. The goal was to validate key banking workflows from both the Bank Manager and Customer perspectives by designing and executing functional test cases.

The testing focused on customer management, account-related operations, deposits, withdrawals, transaction history, validation, search, and navigation.

## 🌐 Application Under Test

Application: XYZ Bank / Banking Project
URL: https://www.globalsqa.com/angularJs-protractor/BankingProject/#/login
Driver URL: https://drive.google.com/drive/folders/1liLZZi5z2HQf6ZRGmocYOBoWW4ZjhAxG

## 🐞Bug Report link: https://drive.google.com/drive/folders/1qyRWXntiVXfNApqgCKEzyFPvyIWZeD45

## 🎯 Testing Objectives

	-- Validate critical banking application workflows.

	-- Verify positive and negative scenarios.

	-- Check form validations and input handling.

	-- Verify customer management functionality.

	-- Validate deposit and withdrawal transactions.

	Verify search, delete, navigation, and transaction-history features.

	Record actual results and test execution status.

	Identify and document failed test scenarios.


## 📊 Test Result

	--Total Test Cases: 35
	--Passed: 31
	--Failed: 4
	--Blocked: 0
	--Overall Pass Rate: 88.57%

The failed scenarios were recorded in the execution sheet with supporting notes/references for further investigation.

## 🛠️ Testing Techniques

The project applied common manual testing techniques, including:
Functional Testing
Positive Testing
Negative Testing
Boundary/Validation Testing
UI Testing
Input Validation Testing
Navigation Testing
Transaction Testing
Search Testing
Regression-oriented verification


## 🐞 Defect Identification

During execution, 4 test cases failed:
Manager Module: 3 failed
Customer Module: 1 failed
Failed cases were marked in the test execution sheet and linked to supporting screenshot/screen-recording references where available.


## 👨‍💻 Project Role

Role: Manual QA / SQA Engineer
Testing Type: Manual Testing
Primary Artifact: Test Case & Test Execution Excel Workbook

## Author

### Mozzammel Hossen
Aspiring SQA Engineer | Manual Testing | API Testing | Automation Testing
